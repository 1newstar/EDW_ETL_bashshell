...
#维护时间：
#运行周期：日
#例    程：perl test.pl d/full date_time schema table_name
#备    注：
###########################################################################
# 一.自定义模块和配置文件
1.vertica DBI模块MyDBI，读取配置文件，进行负载均衡的连接（现有的）
2.oracle DBI模块，根据传入的实例名称，选择IP，用户名，密码，进行数据库连接。
# 二.配置表说明
1.规则定义表
create table 规则定义表
(
规则名称=表名,规则索引名。
源库名=根据实例名获得数据库连接，
规则1=vertica侧的验证sql,又操作符和日期的替换，《》‘yyyymmdd’，count(*)和sum
规则2=oracle侧的验证sql,又操作符和日期的替换，《》‘yyyymmdd’
日期标识，
有效标识
  )
2.匹配结果表
...
#3.自定义全局变量
my $gv_delrows=0;                                    ##程序删除记录行数，默认为0
my $gv_insrows=0;                                    ##程序插入记录行数，默认为0
